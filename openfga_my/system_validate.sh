...
source "${ROOT}/scripts/common.sh" >/dev/null 2>&1 || {
...
preflight() {
  local missing=0
  for f in "${DEX_DIR}/generated/dex.env" "${ROOT}/generated/fga.env" "${VAULT_DIR}/generated/vault.env" "${DEX_DIR}/config.yaml"; do
...
# --- helpers for checks ------------------------------------------------------
clear_tokens() { rm -f "${ROOT}/generated/tokens/"*.json 2>/dev/null || true; }
...
jwt_has_email() {
  ...
  tok=$(LIBCLOUD_USER="$user" LIBCLOUD_PASSWORD="$pass" \
        python3 "${ROOT}/scripts/idp_login.py" 2>/dev/null) || return 1
  [[ -n "$tok" ]] || return 1
...
superadmin_login_succeeds() {
  ...
  [[ -n "$pw" ]] && pw="$(grep -E '^LIBCLOUD_SUPERADMIN_PASSWORD=' "${ROOT}/generated/dex.env" 2>/dev/null | cut -d= -f2- || true)"
  [[ -n "$pw" ]] || return 1
  LIBCLOUD_USER=superadmin LIBCLOUD_PASSWORD="$pw" \
    python3 "${ROOT}/scripts/idp_login.py" 2>/dev/null >"${TMP}/sa.jwt" || return 1
  [[ -s "${TMP}/sa.jwt" ]] || return 1
  SUPERADMIN_JWT="$(cat "${TMP}/sa.jwt")" python3 "${ROOT}/scripts/verify_superadmin_jwt.py" >/dev/null 2>&1
}

openfga_bootstrap_refuses_without_superadmin() {
  # Empty SUPERADMIN_JWT -> bootstrap exits 3 with the gating message.
  SUPERADMIN_JWT="" python3 "${ROOT}/openfga_bootstrap.py" >"${TMP}/ofg.out" 2>&1
  local rc=$?
...
creds_refuses_non_owner() {
  ...
  TENANT="${tenant}" LIBCLOUD_USER="${user}" LIBCLOUD_PASSWORD="${pass}" \
    ...
    python3 "${ROOT}/scripts/set_tenant_credentials.py" >"${TMP}/creds.out" 2>&1
  local rc=$?
  [[ ${rc} -eq 3 ]] && grep -q "not allowed to manage credentials" "${TMP}/creds.out"
}

run_aws()     { clear_tokens; LIBCLOUD_USER="$1" PROVISION="${PROVISION_VMS}" "${ROOT}/scripts/provision_aws.sh"; }
run_nutanix() { clear_tokens; LIBCLOUD_USER="$1" PROVISION="${PROVISION_VMS}" "${ROOT}/scripts/provision_nutanix.sh"; }
