...
Optional/legacy: **Authentik** at `http://localhost:9000` (`AUTHENTIK_URL`),
used only when `IDP_PROVIDER=authentik` (`idp_login.py:24-29, 256-295`) — its
`/api/v3/flows/executor/<slug>/` and `/application/o/token/`.
...
| **Authentik** (legacy) | Flow-based password stage + OIDC token issuance | Per-flow policy | Only when `IDP_PROVIDER=authentik`. |
