#!/usr/bin/env python3
"""Obtain an OIDC access token for libcloud REST provisioning scripts (Dex or Authentik)."""
...
IDP_PROVIDER = os.environ.get("IDP_PROVIDER", "dex").lower()
...
# Authentik (legacy / Phase 2 upstream example)
...
FLOW_SLUG = os.environ.get("AUTHENTIK_FLOW_SLUG", "libcloud-authentication-flow")
...
USER_UID = {
    ...
    # Legacy Authentik/Phase-1 aliases → stable LLDAP uids.
    "admin": "cloud-admin",
...
def authentik_login(username: str, password: str) -> dict:
    jar = CookieJar()
    opener = _cookie_opener(jar)
    executor_url = f"{AUTHENTIK_URL}/api/v3/flows/executor/{FLOW_SLUG}/"
    stage = _request_json(opener, "GET", executor_url)
    for _ in range(10):
        if not stage:
            stage = _request_json(opener, "GET", executor_url)
            continue
        ...
        if component == "ak-stage-identification":
            _request_json(opener, "POST", executor_url, {"component": component, "uid_field": username})
            stage = _request_json(opener, "GET", executor_url)
            continue
        if component == "ak-stage-password":
            stage = _request_json(
                ...
                "POST",
                executor_url,
                {"component": component, "password": password},
            )
            if stage.get("component") in {"xak-flow-redirect", "ak-stage-user-login", "xak-flow-end"}:
                break
            stage = _request_json(opener, "GET", executor_url)
            continue
        if component in {"xak-flow-redirect", "ak-stage-user-login", "xak-flow-end"}:
            break
        raise RuntimeError(f"Unexpected Authentik stage: {component}")
    ...
    params = urllib.parse.urlencode(
        {
            ...
            "state": "libcloud-authentik",
        }
    )
    code = _capture_auth_code(opener, f"{AUTHENTIK_URL}/application/o/authorize/?{params}")
    return _exchange_code(f"{AUTHENTIK_URL}/application/o/token/", code)
...
def login(username: str, password: str) -> dict:
    ...
    token_url = (
        ...
        if IDP_PROVIDER == "dex"
        else f"{AUTHENTIK_URL}/application/o/token/"
    )
    ...
    if IDP_PROVIDER == "authentik":
        token = authentik_login(username, password)
    else:
...
def main() -> int:
    ...
    try:
        ...
        print(f"IdP login failed ({IDP_PROVIDER}): {exc}", file=sys.stderr)
        return 1
