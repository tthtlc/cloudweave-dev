...
# Output: 66 subdirectories under http_flow_examples/, each with:
...
{
  "authorizeUrl": "http://login.cloudweave.xyz:5556/dex/auth?client_id=libcloud-portal&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fauth%2Fcallback&response_type=code&scope=openid+profile+email&state=aB3kXp9qR7sW2vY6uN1mF4dG8jL0oQ5t&code_challenge=KzP9wLm2xR7vQ4nJ8sA1dF6gH0kT3yB5&code_challenge_method=S256&connector_id=lldap",
...
Host: login.cloudweave.xyz:5556
...

{
  ...
  "iss": "http://login.cloudweave.xyz:5556/dex",
  "aud": "libcloud-rest"
}
EOF
}
...
# ---------------------------------------------------------------------------
# STAGE 8: Company / Department Management (Browser → Identity Service)
# ---------------------------------------------------------------------------
gen_39_create_company() {
  _section "39 — POST /api/companies (Browser → Identity Service, Create Company — SuperAdmin)"
  _dir "39-create-company"

  _req "39-create-company" << 'EOF'
...
{
  "name": "Acme Corp",
  "adminUserId": "int-acme-admin"
}
EOF

  _resp "39-create-company" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 68
Date: Wed, 23 Jul 2026 14:28:01 GMT
...
gen_40_list_companies() {
  _section "40 — GET /api/companies (Browser → Identity Service, List Companies — SuperAdmin)"
  _dir "40-list-companies"

  _req "40-list-companies" << 'EOF'
...
EOF
  ...
  _resp "40-list-companies" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 330
Date: Wed, 23 Jul 2026 14:28:02 GMT
...
gen_41_list_assignable_users() {
  _section "41 — GET /api/users/assignable (Browser → Identity Service, Assignable Users — CompanyAdmin)"
  _dir "41-list-assignable-users"

  _req "41-list-assignable-users" << 'EOF'
GET /api/users/assignable HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...company_admin_session...

(empty body — GET request)
EOF

  _resp "41-list-assignable-users" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 460
Date: Wed, 23 Jul 2026 14:28:03 GMT

{
  "users": [
    {
      "internalUserId": "int-eng-owner",
      "email": "eng-owner@libcloud.local",
    ...
    {
      "internalUserId": "int-eng-viewer",
      "email": "eng-viewer@libcloud.local",
...
gen_42_create_department() {
  _section "42 — POST /api/companies/{id}/departments (Browser → Identity Service, Create Multi-Provider Department)"
  _dir "42-create-department"

  _req "42-create-department" << 'EOF'
POST /api/companies/acme/departments HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...company_admin_session...

{
  "name": "Engineering",
  "clouds": ["aws", "nutanix"],
  "ownerUserId": "int-eng-owner",
  "credentials": {
    "aws": {
      "key": "AKIAYHGEH2P75YEO4GGY",
      "secret": "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"
    },
    "nutanix": {
      "key": "admin",
      "secret": "admin",
      "host": "host.docker.internal:9442"
    }
  }
}
EOF

  _resp "42-create-department" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 72
Date: Wed, 23 Jul 2026 14:28:04 GMT
...
gen_43_list_departments() {
  _section "43 — GET /api/companies/{id}/departments (Browser → Identity Service, List Departments)"
  _dir "43-list-departments"

  _req "43-list-departments" << 'EOF'
GET /api/companies/acme/departments HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...company_admin_session...
...
EOF
  ...
  _resp "43-list-departments" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 140
Date: Wed, 23 Jul 2026 14:28:05 GMT
...
gen_44_get_department_credential() {
  _section "44 — GET /api/departments/{dept}/credential (Browser → Identity Service, View Credential — Owner)"
  _dir "44-get-department-credential"

  _req "44-get-department-credential" << 'EOF'
GET /api/departments/eng/credential HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...department_owner_session...

(empty body — GET request)
EOF

  _resp "44-get-department-credential" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 128
Date: Wed, 23 Jul 2026 14:28:06 GMT
...
gen_45_rotate_department_credential() {
  _section "45 — PUT /api/departments/{dept}/credential (Browser → Identity Service, Rotate Credential — Owner)"
  _dir "45-rotate-department-credential"

  _req "45-rotate-department-credential" << 'EOF'
PUT /api/departments/eng/credential HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...department_owner_session...
...
{
  "key": "AKIAYHGEH2P7QJKE5UGO",
  "secret": "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"
}
EOF

  _resp "45-rotate-department-credential" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 50
Date: Wed, 23 Jul 2026 14:28:07 GMT
...
gen_46_update_company() {
  _section "46 — PUT /api/companies/{id} (Browser → Identity Service, Rename/Re-admin Company — SuperAdmin)"
  _dir "46-update-company"

  _req "46-update-company" << 'EOF'
PUT /api/companies/acme HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...

{
  ...
  "adminUserId": "int-acme-admin-2"
}
EOF

  _resp "46-update-company" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 58
Date: Wed, 23 Jul 2026 14:28:08 GMT

{
  "id": "acme-corp-international",
  "updated": true
}
EOF
}

gen_47_delete_company() {
  _section "47 — DELETE /api/companies/{id} (Browser → Identity Service, Delete Company — SuperAdmin)"
  _dir "47-delete-company"

  _req "47-delete-company" << 'EOF'
DELETE /api/companies/acme-corp-international HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...
...
EOF
  ...
  _resp "47-delete-company" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 52
Date: Wed, 23 Jul 2026 14:28:09 GMT
...
gen_48_update_department() {
  _section "48 — PUT /api/departments/{dept} (Browser → Identity Service, Edit Providers/Owner — CompanyAdmin)"
  _dir "48-update-department"

  _req "48-update-department" << 'EOF'
PUT /api/departments/eng HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...company_admin_session...
...
EOF
  ...
  _resp "48-update-department" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 44
Date: Wed, 23 Jul 2026 14:28:10 GMT
...
gen_49_delete_department() {
  _section "49 — DELETE /api/departments/{dept} (Browser → Identity Service, Delete Department — CompanyAdmin)"
  _dir "49-delete-department"

  _req "49-delete-department" << 'EOF'
DELETE /api/departments/eng HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...company_admin_session...
...
EOF
  ...
  _resp "49-delete-department" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 44
Date: Wed, 23 Jul 2026 14:28:11 GMT
...
gen_50_list_company_members() {
  _section "50 — GET /api/companies/{id}/members (Browser → Identity Service, List Department Members — CompanyAdmin)"
  _dir "50-list-company-members"

  _req "50-list-company-members" << 'EOF'
GET /api/companies/acme/members HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...company_admin_session...
...
EOF
  ...
  _resp "50-list-company-members" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 330
Date: Wed, 23 Jul 2026 14:28:12 GMT
...
gen_51_assign_department_user() {
  _section "51 — PUT /api/departments/{dept}/users/{uid} (Browser → Identity Service, Assign/Move Member — CompanyAdmin)"
  _dir "51-assign-department-user"

  _req "51-assign-department-user" << 'EOF'
PUT /api/departments/eng/users/int-eng-viewer HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...company_admin_session...
...
EOF
  ...
  _resp "51-assign-department-user" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 62
Date: Wed, 23 Jul 2026 14:28:13 GMT
...
gen_52_remove_department_user() {
  _section "52 — DELETE /api/departments/{dept}/users/{uid} (Browser → Identity Service, Remove Member — CompanyAdmin)"
  _dir "52-remove-department-user"

  _req "52-remove-department-user" << 'EOF'
DELETE /api/departments/eng/users/int-eng-viewer HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...company_admin_session...
...
EOF
  ...
  _resp "52-remove-department-user" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 66
Date: Wed, 23 Jul 2026 14:28:14 GMT
...
# ---------------------------------------------------------------------------
# STAGE 9: OpenFGA Explorer (Browser → Identity Service → OpenFGA, SuperAdmin)
# ---------------------------------------------------------------------------
gen_53_openfga_store() {
  _section "53 — GET /api/openfga/store (Browser → Identity Service → OpenFGA, Store Metadata — SuperAdmin)"
  _dir "53-openfga-store"

  _req "53-openfga-store" << 'EOF'
GET /api/openfga/store HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...

(empty body — GET request; identity service proxies GET /stores/{store_id})
EOF

  _resp "53-openfga-store" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 150
Date: Wed, 23 Jul 2026 14:29:01 GMT

{
  "id": "01J5K7M9P2R4V6W8",
  "name": "libcloud-rest-store",
  "created_at": "2026-07-01T00:00:00Z",
  "updated_at": "2026-07-23T14:22:00Z"
}
EOF
}

gen_54_openfga_models() {
  _section "54 — GET /api/openfga/models (Browser → Identity Service → OpenFGA, List Models — SuperAdmin)"
  _dir "54-openfga-models"

  _req "54-openfga-models" << 'EOF'
GET /api/openfga/models HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...

(empty body — GET request; proxies GET /stores/{store_id}/authorization-models)
EOF

  _resp "54-openfga-models" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 320
Date: Wed, 23 Jul 2026 14:29:02 GMT

{
  "authorization_models": [
    {
      "id": "01M1ZZEQE73M3NSSK0JQ837BF0",
      "schema_version": "1.1",
      "type_definitions": [
        {"type": "user"},
        {"type": "company"},
...
gen_55_openfga_model() {
  _section "55 — GET /api/openfga/models/{id} (Browser → Identity Service → OpenFGA, Single Model — SuperAdmin)"
  _dir "55-openfga-model"

  _req "55-openfga-model" << 'EOF'
GET /api/openfga/models/01M1ZZEQE73M3NSSK0JQ837BF0 HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...
...
EOF
  ...
  _resp "55-openfga-model" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 290
Date: Wed, 23 Jul 2026 14:29:03 GMT
...
gen_56_openfga_assertions() {
  _section "56 — GET /api/openfga/assertions/{model_id} (Browser → Identity Service → OpenFGA, Assertions — SuperAdmin)"
  _dir "56-openfga-assertions"

  _req "56-openfga-assertions" << 'EOF'
GET /api/openfga/assertions/01M1ZZEQE73M3NSSK0JQ837BF0 HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...
...
EOF
  ...
  _resp "56-openfga-assertions" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 210
Date: Wed, 23 Jul 2026 14:29:04 GMT
...
gen_57_openfga_changes() {
  _section "57 — GET /api/openfga/changes (Browser → Identity Service → OpenFGA, Change Log — SuperAdmin)"
  _dir "57-openfga-changes"

  _req "57-openfga-changes" << 'EOF'
GET /api/openfga/changes?page_size=50 HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...

(empty body — GET request; proxies GET /stores/{store_id}/changes)
EOF

  _resp "57-openfga-changes" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 330
Date: Wed, 23 Jul 2026 14:29:05 GMT
  ...
  "changes": [
    {
      ...
      "timestamp": "2026-07-23T14:28:04Z"
    }
  ],
  "continuation_token": ""
}
...
gen_58_openfga_list_users() {
  _section "58 — POST /api/openfga/list-users (Browser → Identity Service → OpenFGA, List Users — SuperAdmin)"
  _dir "58-openfga-list-users"

  _req "58-openfga-list-users" << 'EOF'
POST /api/openfga/list-users HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...
...
EOF
  ...
  _resp "58-openfga-list-users" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 170
Date: Wed, 23 Jul 2026 14:29:06 GMT

{
  "users": [
    {"object": {"type": "user", "id": "eng-owner"}},
    {"object": {"type": "user", "id": "eng-viewer"}}
  ]
}
EOF
}

gen_59_openfga_list_objects() {
  _section "59 — POST /api/openfga/list-objects (Browser → Identity Service → OpenFGA, List Objects — SuperAdmin)"
  _dir "59-openfga-list-objects"

  _req "59-openfga-list-objects" << 'EOF'
POST /api/openfga/list-objects HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...
...
EOF
  ...
  _resp "59-openfga-list-objects" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 40
Date: Wed, 23 Jul 2026 14:29:07 GMT
...
gen_60_openfga_expand() {
  _section "60 — POST /api/openfga/expand (Browser → Identity Service → OpenFGA, Expand Relationship — SuperAdmin)"
  _dir "60-openfga-expand"

  _req "60-openfga-expand" << 'EOF'
POST /api/openfga/expand HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...
...
EOF
  ...
  _resp "60-openfga-expand" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 280
Date: Wed, 23 Jul 2026 14:29:08 GMT

{
  "tree": {
    "root": {
      "name": "tenant:eng#can_read",
      "leaf": {
        "users": {
          "users": ["user:eng-owner"]
        }
      }
    }
  }
...
gen_61_openfga_rest_api_policies() {
  _section "61 — GET /api/openfga/rest-api-policies (Browser → Identity Service, REST Policy Table — SuperAdmin)"
  _dir "61-openfga-rest-api-policies"

  _req "61-openfga-rest-api-policies" << 'EOF'
GET /api/openfga/rest-api-policies HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...superadmin_session...

(empty body — GET request; returns the libcloud REST API policies.json table)
EOF

  _resp "61-openfga-rest-api-policies" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 420
Date: Wed, 23 Jul 2026 14:29:09 GMT
...
# ---------------------------------------------------------------------------
# STAGE 10: Health + Cloud Extras (Browser → Identity Service)
# ---------------------------------------------------------------------------
gen_62_health() {
  _section "62 — GET /health (Browser → Identity Service, Health Check)"
  _dir "62-health"
...
Content-Type: application/json
Content-Length: 18
Date: Wed, 23 Jul 2026 14:30:01 GMT
...
gen_63_list_hosts() {
  _section "63 — GET /api/hosts/{cloud} (Browser → Identity Service → REST, Physical Host Details — Nutanix)"
  _dir "63-list-hosts"

  _req "63-list-hosts" << 'EOF'
GET /api/hosts/nutanix HTTP/1.1
Host: localhost:8766
Accept: application/json
Cookie: libcloud_portal_sid=...department_owner_session...

(empty body — GET request; proxies GET /v1/compute/hosts)
EOF

  _resp "63-list-hosts" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 400
Date: Wed, 23 Jul 2026 14:30:02 GMT
...
gen_64_provision_private() {
  _section "64 — POST /api/provision-private/{cloud} (Browser → Identity Service, Provision Bastion + Private VM Pair)"
  _dir "64-provision-private"

  _req "64-provision-private" << 'EOF'
POST /api/provision-private/aws HTTP/1.1
Host: localhost:8766
Content-Type: application/json
Accept: application/json
Cookie: libcloud_portal_sid=...department_owner_session...
...
{
  "vmName": "libcloud-aws-pair-1753486921"
}
EOF

  _resp "64-provision-private" << 'EOF'
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 260
Date: Wed, 23 Jul 2026 14:30:03 GMT

{
  "provider": "aws",
  "vmName": "libcloud-aws-pair-1753486921",
  "bastionName": "libcloud-aws-pair-1753486921-bastion",
  "internalName": "libcloud-aws-pair-1753486921-internal",
  "status": "provisioned",
  "message": "provision_aws_private.sh exit=0",
  "exitCode": 0,
  "stdout": "",
  "stderr": ""
}
...
main() {
  ...
  rm -rf "$ROOT"/0[1-9]* "$ROOT"/1[0-9]* "$ROOT"/2[0-9]* "$ROOT"/3[0-9]* \
    "$ROOT"/4[0-9]* "$ROOT"/5[0-9]* "$ROOT"/6[0-9]*
  COUNT=0
  ...
  # Stage 8: Company / Department Management
  gen_39_create_company
  gen_40_list_companies
  gen_41_list_assignable_users
  gen_42_create_department
  ...
  # Stage 9: OpenFGA Explorer (SuperAdmin)
  gen_53_openfga_store
  gen_54_openfga_models
  gen_55_openfga_model
  gen_56_openfga_assertions
