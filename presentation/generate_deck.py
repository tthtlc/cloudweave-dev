...
bullets_slide(
    ...
    [
        ("OpenFGA v1.16.0, PostgreSQL-backed, 8 types on schema 1.1", 0),
        ("user · platform · tenant · libcloud_api · provider · resource_class · aws_region · nutanix_cluster", 1),
        ("Bootstrap seeds 48 tuples — 39 structural wiring + 9 role grants", 1),
...
flow_slide(
    ...
    [
        ...
        ("REST → Vault", "GET /v1/secret/data/libcloud/aws   X-Vault-Token   (30 s cache)"),
        ("REST → cloud", "Apache libcloud driver → AWS EC2 or Nutanix Prism Central"),
    ],
    n, "The client names which credential to use (auth_binding); it never supplies the value.",
)
...
bullets_slide(
    ...
    [
        ("Vault 1.15, file storage backend, KV v2 mounted at secret/", 0),
        ("secret/data/libcloud/aws       → AWS access key / secret", 1),
        ("secret/data/libcloud/nutanix   → Prism username / password", 1),
        ("Writing a tenant credential is itself authorized", 0),
        ("The tenant owner logs into Dex, passes an OpenFGA can_manage_credentials check,", 1),
        ("and only then writes to Vault", 1),
        ("Reading happens server-side at provision time", 0),
        ("libcloud.rest holds a scoped read token and caches values for 30 s", 1),
        ("Client-supplied credentials are rejected with 403 unless explicitly enabled", 1),
        ("The deployed posture is development-grade", 0),
        ("Single-share unseal (threshold 1), root token on disk, TLS disabled, no audit device", 1),
    ],
    n, FOOT,
)
