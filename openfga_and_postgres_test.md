...
serving correct responses via `curl` against the OpenFGA REST API on
`http://localhost:8080`.

## Test Results (live run 2026-07-28)

```
╔══════════════════════════════════════════════════════╗
║   OpenFGA + Postgres Container Test                 ║
║   FGA URL: http://localhost:8080                     ║
║   PG: openfga-postgres                   openfga      ║
║   Auth: none (auth'd tests skipped)                 ║
╚══════════════════════════════════════════════════════╝

── 0. Docker containers ──
✓ Container openfga-postgres is running
✓ Container openfga is running

── 1. Health & readiness (unauthenticated) ──
✓ GET /healthz → 200 (HTTP 200)
✓ GET /readyz → 404 (not yet available in this version — acceptable)
✓ GET /healthz returns valid JSON (HTTP 200, body matches 'status')

── 2. Authentication enforcement ──
✓ GET /stores without token → 401 or 200 (HTTP 401, body matches 'code')

── 3. Store & model (authenticated) — skipped, no auth token ──
⚠ Skipped: authenticated tests require FGA_API_TOKEN, SUPERADMIN_JWT,
⚠   or an unexpired generated/tokens/superadmin.jwt

── 4. Tuple operations (authenticated) — skipped ──
⚠ Skipped: no auth token available

── 5. Expand & list-objects (authenticated) — skipped ──
⚠ Skipped: no auth token available

── 6. Response format ──
✓ Content-Type is application/json (healthz)
✓ Response body is valid JSON (healthz)
✓ POST /stores/{id}/check with bad JSON → 400 (HTTP 400)

── 7. Metrics endpoint ──
✓ GET /metrics → 200 + openfga metrics (HTTP 200, body matches 'openfga')
...
| Section | What it tests | Auth | Count |
|---|---|---|---|
| 0. Container check | `openfga-postgres` and `openfga` running via `docker ps` | no | 2 |
| 1. Health & readiness | `/healthz` → 200 + valid JSON; `/readyz` graceful handling of absence in v1.16.0 | no | 3 |
| 2. Auth enforcement | GET `/stores` without token → 401 (OIDC authn is on) | no | 1 |
| 3. Store & model | list stores, paginated, get by ID, 404 for nonexistent, list/get authorization models | yes | 6 |
| 4. Tuple read/check | read all tuples, read filtered, 6 Check() assertions covering superadmin/owner/admin/viewer/denied, missing model_id → 400, empty write → 200 | yes | 9 |
| 5. Expand & list-objects | expand relation tree, list objects a user can read | yes | 2 |
| 6. Response format | Content-Type JSON, valid JSON parse, bad JSON → 400 | no | 3 |
| 7. Metrics | Prometheus `/metrics` on port 2112 | no | 1 |

**Total: 27 checks** (17 unauthenticated + 10 authenticated). Authenticated tests
skip gracefully when no valid token is found.
...
```bash
# Defaults: localhost:8080, containers openfga-postgres + openfga
./openfga_postgres/scripts/fga-test.sh

# Custom FGA URL
./openfga_postgres/scripts/fga-test.sh http://192.168.1.50:8080

# With a fresh auth token (enables all 27 checks)
FGA_API_TOKEN=<token> ./openfga_postgres/scripts/fga-test.sh
SUPERADMIN_JWT=<jwt>  ./openfga_postgres/scripts/fga-test.sh

# Or: regenerate a fresh superadmin token, then run
...
The script matches `fga_auth.sh` token resolution (priority order):
1. `$FGA_API_TOKEN` — preshared-key mode or explicit override
2. `$SUPERADMIN_JWT` — set by `superadmin_auth.sh`
3. `openfga_postgres/generated/tokens/superadmin.jwt` — only if unexpired

Without a valid token, authenticated tests (sections 3-5) skip with a warning.
No tests fail due to missing auth.

## OpenFGA REST API notes (discovered while writing this)

- `/healthz` returns `{"status":"SERVING"}` — always available, unauthenticated
- `/readyz` was added after v1.16.0; the script accepts either 200 or 404
- `GET /stores` without auth → 401 with `{"code":"unauthenticated"}` when `authn-method=oidc`
- `/metrics` on port 2112 is unauthenticated Prometheus scrape endpoint
- `POST /stores/{id}/check` requires `authorization_model_id` in the body — omitting it gets a 400
- `POST /stores/{id}/read` and `POST /stores/{id}/write` use POST with JSON body (not GET), with `page_size` for pagination
...
```
openfga_postgres/
├── docker-compose.yml         # postgres + openfga-migrate + openfga + bootstrap
├── Dockerfile                 # multi-stage: downloads + verifies OpenFGA binary
├── .env                       # defaults: ports, OIDC issuer, credentials
├── openfga_bootstrap.py       # idempotent: store, model, tuples, validation
├── fga_auth.sh                # shared auth/config sourcing (sourced by list_users.sh)
├── scripts/
│   └── fga-test.sh            # ← this test script
└── generated/
    ├── fga.env                # FGA_STORE_ID, FGA_MODEL_ID, FGA_API_URL
    └── tokens/superadmin.jwt  # cached bearer token for test_script tools
```

Exit code = number of failures, so `exit 0` means everything passed — usable in CI.
