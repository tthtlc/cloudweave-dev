#!/usr/bin/env bash
# Shared curl helpers for Dex (IdP) + OpenFGA + libcloud REST provisioning demos.
#
...
# OpenFGA JWKS freshness: handled in-process by OpenFGA itself. We pin v1.16.0
# (see openfga_postgres/Dockerfile), whose OIDC authenticator sets
# `RefreshUnknownKID: true` (rate-limited to 1/min) on the keyfunc JWKS cache,
# plus a 48h periodic refresh (PR #3101). So when Dex rotates its signing key
# (every 6h, storage: memory), the next token's unknown `kid` makes OpenFGA
# refetch Dex's JWKS itself — no container restart, no helper script. The old
# restart-based openfga_ensure_fresh.sh workaround was removed; do not
# reintroduce it unless the pinned image is downgraded below v1.16.0.
...
# Step 1: Authenticate to Dex (IdP) and obtain an OIDC access token.
idp_login() {
