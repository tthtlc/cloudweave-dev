...
for f in "${REPO_ROOT}/test_script/generated/vault.env" "${REPO_ROOT}/.env"; do
  [[ -f "$f" ]] || continue
...
NOW_ISO=$(fga_now)
OPENFGA_ENV="${REPO_ROOT}/openfga_postgres/.env"
LIBCLOUD_REST_ENV="${REPO_ROOT}/libcloud.rest/.env"
...
if [[ "$APPLY" -ne 1 ]]; then
  ...
  echo "  docker compose -f ${REPO_ROOT}/libcloud.rest/docker-compose.yml restart ${LIBCLOUD_REST_CONTAINER:-libcloud-rest-api}"
