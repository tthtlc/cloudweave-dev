...
| **Authentik** *(optional)* | Docker profile `authentik` | 9000 | PostgreSQL + Redis | Phase 2 upstream IdP example only — not wired into `idp_login.py` (Dex-only); use via a Dex connector |
