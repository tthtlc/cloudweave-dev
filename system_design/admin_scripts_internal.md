...
- **Purpose:** Foundational bash library for all non-LLDAP scripting. Loads env files (`.env`, `dex.env`, `fga.env`), resolves the libcloud user password, and provides HTTP, JSON, IdP auth, OpenFGA check, and libcloud REST API helpers.
...
- **Env vars loaded:** `.env`, `../dex/generated/dex.env`, `generated/fga.env`
- **Requires:** `curl`, `python3`, `LIBCLOUD_OIDC_CLIENT_SECRET`, `FGA_STORE_ID`, `FGA_MODEL_ID`, Dex URL, `LIBCLOUD_PASSWORD` (or per-role variant)
...
- **Purpose:** Obtain an OIDC access token from Dex via the authorization code flow. This is the **token-acquisition primitive** used by all other scripts.
- **WTD mapping:** Cross-cutting — all authenticated operations depend on it.
- **How it works:**
  1. Builds the Dex authorize URL, starts a local HTTP server on `127.0.0.1:8766`, POSTs credentials to the Dex login form, captures the authorization code, and exchanges it for tokens at `/dex/token`.
  2. Token caching: checks `generated/tokens/{user}.json` for a refresh token; attempts a silent refresh before performing a full re-login.
  3. Prints `access_token` to stdout on success.
- **Env vars:** `DEX_URL`, `LIBCLOUD_OIDC_CLIENT_ID`, `LIBCLOUD_OIDC_CLIENT_SECRET` (**required** — exits if blank), `LIBCLOUD_OIDC_REDIRECT_URI`, `LIBCLOUD_USER`, `LIBCLOUD_PASSWORD` (**required**), `IDP_TOKEN_CACHE_DIR`, `IDP_LOGIN_VERBOSE`/`VERBOSE`
